...
export function encodeText({ text }) { return text || ''; }
...
export function encodePhone({ phone }) { return phone ? `tel:${phone}` : ''; }
...
export function encodeVCard({ firstName='', lastName='', org='', position='', phoneWork='',
  phonePrivate='', phoneMobile='', email='', website='', street='', zipcode='',
  city='', state='', country='', version='3' }) {
  const lines = ['BEGIN:VCARD'];
  lines.push(`VERSION:${version==='2'?'2.1':version==='4'?'4.0':'3.0'}`);
  const fn = escVCard(firstName), ln = escVCard(lastName);
  if (fn || ln) { lines.push(`N:${ln};${fn};;;`); lines.push(`FN:${(fn+' '+ln).trim()}`); }
  if (org)          lines.push(`ORG:${escVCard(org)}`);
  if (position)     lines.push(`TITLE:${escVCard(position)}`);
  const v4 = version==='4', v2 = version==='2';
...
const ENCODERS = {
  text: encodeText, url: encodeUrl, email: encodeEmail, phone: encodePhone,
  sms: encodeSms, wifi: encodeWifi, vcard: encodeVCard, location: encodeLocation, event: encodeEvent,
};
...
export function detectDataType(raw) {
  ...
  if (/^BEGIN:VCARD/i.test(raw)) {
    ...
    if (n) { const p = n.slice(2).split(';'); d.lastName = p[0]||''; d.firstName = p[1]||''; }
    const fn = lines.find(l => /^FN:/i.test(l));
    if (fn && !d.firstName && !d.lastName) { const p = fn.slice(3).trim().split(' '); d.firstName=p[0]||''; d.lastName=p.slice(1).join(' ')||''; }
    const org = lines.find(l => /^ORG:/i.test(l)); if (org) d.org = org.slice(4).trim();
    const ttl = lines.find(l => /^TITLE:/i.test(l)); if (ttl) d.position = ttl.slice(6).trim();
    for (const l of lines) {
      if (/^TEL[^:]*(?:TYPE=WORK|WORK)[^:]*:/i.test(l)) d.phoneWork = l.slice(l.indexOf(':')+1).replace(/^tel:/i,'').trim();
      else if (/^TEL[^:]*(?:TYPE=HOME|HOME)[^:]*:/i.test(l)) d.phonePrivate = l.slice(l.indexOf(':')+1).replace(/^tel:/i,'').trim();
      else if (/^TEL[^:]*(?:TYPE=CELL|CELL|MOBILE)[^:]*:/i.test(l)) d.phoneMobile = l.slice(l.indexOf(':')+1).replace(/^tel:/i,'').trim();
      else if (/^TEL[^:]*:/i.test(l) && !d.phoneWork && !d.phonePrivate && !d.phoneMobile) d.phoneMobile = l.slice(l.indexOf(':')+1).replace(/^tel:/i,'').trim();
    }
    const em = lines.find(l => /^EMAIL[^:]*:/i.test(l)); if (em) d.email = em.slice(em.indexOf(':')+1).trim();
    const url = lines.find(l => /^URL[^:]*:/i.test(l)); if (url) d.website = url.slice(url.indexOf(':')+1).trim();
    const adr = lines.find(l => /^ADR[^:]*:/i.test(l));
    if (adr) { const p = adr.slice(adr.indexOf(':')+1).split(';'); if (p.length>=7) { d.street=p[2];d.city=p[3];d.state=p[4];d.zipcode=p[5];d.country=p[6]; } }
  ...
  if (/^mailto:/i.test(raw)) {
    ...
    if (qs) { const p = new URLSearchParams(qs); d.subject=p.get('subject')||''; d.body=p.get('body')||''; d.cc=p.get('cc')||''; d.bcc=p.get('bcc')||''; }
    return { type: 'email', data: d };
  }
  if (/^tel:/i.test(raw)) return { type: 'phone', data: { phone: raw.replace(/^tel:/i,'') } };
  if (/^SMSTO:/i.test(raw)) { const p=raw.replace(/^SMSTO:/i,'').split(':'); return { type:'sms', data:{ phone:p[0]||'', message:p[1]||'' } }; }
  if (/^WIFI:/i.test(raw)) {
    const d = {};
    const s=raw.match(/S:([^;]*);/i); if(s) d.ssid=s[1];
    const t=raw.match(/T:([^;]*);/i); if(t) d.encryption=t[1].toUpperCase()==='NOPASS'?'nopass':t[1].toUpperCase()==='WEP'?'WEP':'WPA';
    const p=raw.match(/P:([^;]*);/i); if(p) d.password=p[1];
    const h=raw.match(/H:(true|false);/i); d.hidden=h?h[1]==='true':false;
    return { type:'wifi', data: d };
  }
  if (/^geo:/i.test(raw)) { const [lat,lon]=raw.replace(/^geo:/i,'').split(','); return { type:'location', data:{ latitude:lat||'', longitude:lon||'' } }; }
  if (/BEGIN:VCALENDAR|BEGIN:VEVENT/i.test(raw)) {
    const d = {};
    const s=raw.match(/SUMMARY:([^\n\r]*)/i); if(s) d.title=s[1];
    const l=raw.match(/LOCATION:([^\n\r]*)/i); if(l) d.location=l[1];
    const ds=raw.match(/DTSTART(?:[^:]*):([^\n\r]*)/i); if(ds) d.startTime=fromICalDT(ds[1]);
    const de=raw.match(/DTEND(?:[^:]*):([^\n\r]*)/i); if(de) d.endTime=fromICalDT(de[1]);
